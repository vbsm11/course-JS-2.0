...
function t7() {
    let out7 = Math.floor(Math.random() * a7.length);
    document.querySelector('.out-7').textContent = a7[out7];
}
...
function t8(event) {
    let a = 0;
    for (let key in a8) {
        if (key == event.key) a = 1;
    }
    if (a == 0) out8 += event.key;
    if (a == 1) out8 += a8[event.key];
...
function t10(event) {
    ...
    if (event.keyCode == 38 || event.keyCode == 40) h++;
    if (event.keyCode == 37 || event.keyCode == 39) w++;
...
div4.setAttribute('data', '4');
document.querySelector('.keyboard[data="3"]').after(div4);

const kb = document.querySelectorAll('.keyboard');

function t11(event) {
    console.log(event.key);
    for (let i = 0; i < kb.length; i++) {
        kb[i].classList.remove('active');
    }
    let key = event.key;
